...
// next video to implement the Dijkstra algorithm !!!
	public void calculateShortestDistances(int startIndex) {
		// node startIndex as source
		this.nodes[startIndex].setDistanceFromSource(0);
...
//			output += ("\nThe shortest distance from node 0 to node " + i + " is " + nodes[i].getDistanceFromSource());
	...
	public void getShortestRoute(String from,String to) {
		System.out.println("Shortest Route......");
		calculateShortestDistances(placeArray.indexOf(from));
		printShortRoute(placeArray.indexOf(from),placeArray.indexOf(to));
	}

	private void printShortRoute(int from, int to) {
		for (int i = 0; i < this.nodes.length; i++) {
			if(i == to) {				
				System.out.println("The shortest distance from "+nodes[from].getNodeName()+" to " + nodes[i].getNodeName() + " is " + nodes[i].getDistanceFromSource());
			}
		}
	}
}